...
public class RagdollControl : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        foreach (Component c in components)
        {
            ...
            if (c.GetComponent<Rigidbody>() && c.gameObject.layer == 8)
            {
                ...
                {
                    RagBone r = c.gameObject.AddComponent<RagBone>();
                    r.onCollisionEnter.AddListener(OnCollision);
                    r.onTriggerEnter.AddListener(TriggerEnter);
                    r.onTriggerExit.AddListener(TriggerExit);
                    r.onCollisionStay.AddListener(CollisionStay);
    ...
    void OnCollision(Collision collision,GameObject obj){
        ...
        {
            collisionCount++;
            if (collisionCount > bodyParts.Count / 4)
            {
                if (onCollisionEnter != null) onCollisionEnter.Invoke(collision, obj);
            }
        }
    }

    void CollisionStay(Collision collision,GameObject obj){
        if(state==RagdollState.ragdolled){
            collisionCount++;
            if(collisionCount>bodyParts.Count/2){
                if (onCollisionStay != null) onCollisionStay.Invoke(collision, obj);
            }
        }
    }

    void TriggerEnter(Collider other, GameObject obj){
        if(state==RagdollState.ragdolled){
            triggerCount++;
            if(triggerCount>bodyParts.Count/10){
                triggerCount = 0;
                if (onTriggerEnter != null) onTriggerEnter.Invoke(other, obj);
            }
        }
    }

    void TriggerExit(Collider other, GameObject obj)
    {
        if (state == RagdollState.ragdolled)
        {
            triggerCount++;
            ...
            {
                triggerCount = 0;
                if (onTriggerExit != null) onTriggerExit.Invoke(other, obj);
            }
        }
    }
    ...
    {
        foreach (RagBone r in GetComponentsInChildren<RagBone>())
        {
